...
public class RobotControllerAgent : Agent
{
   ...
   public override void Initialize()
   {
      ...
      if (!trainingMode) MaxStep = 01;
   }

   private void ResetAllAxis()
   {
      
      armAxes[0].transform.localRotation = Quaternion.Euler(0.0f, 0.0f, Random.Range(-90.0f,90.0f));
      armAxes[1].transform.localRotation = Quaternion.Euler(00.0f, 0.0f, Random.Range(0.0f,90.0f));
      armAxes[2].transform.localRotation = Quaternion.Euler(0.0f, 0.0f, Random.Range(-90.0f, 90.0f));
      armAxes[3].transform.localRotation = Quaternion.Euler(0.0f, 0.0f, Random.Range(-90.0f, 90.0f));
      armAxes[4].transform.localRotation = Quaternion.Euler(0.0f, 0.0f, Random.Range(-90.0f, 90.0f));
      if(armAxes[1].transform.position.y<0 || armAxes[2].transform.position.y<0 || armAxes[3].transform.position.y<0 || armAxes[4].transform.position.y<0 || endEffector.transform.position.y < 0){
         ResetAllAxis();
      }
   }
   ...
   public override void CollectObservations(VectorSensor sensor)
   {
      ...
      Vector3 toComponent = (target.transform.position - endEffector.transform.TransformPoint(Vector3.zero));
      sensor.AddObservation(toComponent.normalized);
      sensor.AddObservation(Vector3.Distance(target.transform.position,endEffector.transform.TransformPoint(Vector3.zero)));
   ...
   public void JackpotReward(Collider other)
   {
      if (other.transform.CompareTag("Target"))
      {
         ...
         float bonus = Mathf.Clamp01(Vector3.Dot(target.transform.up.normalized,
                          endEffector.transform.up.normalized));
         float reward = SuccessReward + bonus;
         if (float.IsInfinity(reward) || float.IsNaN(reward)){
            Debug.Log("saved reward is either infinity or NaN");
            return;
         }
          Debug.LogWarning("Great! Target reached. Positive reward:" + reward );
         AddReward(reward);
         //EndEpisode();
         UpdateNearestComponent();
      }
...
// Activate below function only during testing

   private void OnCollisionEnter(Collision col){
      if(col.transform.CompareTag("Target")){
         Debug.LogWarning("Great! Target reached");
      }
   }
   ...
   private void Update()
   {
      if(nearestComponent != null)
         Debug.DrawLine(endEffector.transform.position,target.transform.position, Color.green);
   }
}
